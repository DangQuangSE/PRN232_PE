...
namespace givenAPI.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public ActionResult<List<ProductResponse>> SearchProducts(
            string? name,
            string? material,
            int? designerId,
            int? fromYear,
            int? toYear)
        {
            var products = DataInitializer.Products.AsEnumerable();

            if (!string.IsNullOrWhiteSpace(name))
                products = products.Where(p =>
                    p.Name.Contains(name.Trim(), StringComparison.OrdinalIgnoreCase));

            if (!string.IsNullOrWhiteSpace(material))
                products = products.Where(p =>
                    p.Material.Contains(material.Trim(), StringComparison.OrdinalIgnoreCase));

            if (designerId.HasValue)
                products = products.Where(p => p.DesignerId == designerId.Value);

            if (fromYear.HasValue)
                products = products.Where(p =>
                    p.LaunchDate.HasValue && p.LaunchDate.Value.Year >= fromYear.Value);

            if (toYear.HasValue)
                products = products.Where(p =>
                    p.LaunchDate.HasValue && p.LaunchDate.Value.Year <= toYear.Value);

            return Ok(products.Select(ProductToResponse).ToList());
        }
        ...
        public IActionResult DeleteProduct(int id)
        {
            ...
            DataInitializer.ProductReviewers.RemoveAll(pr => pr.ProductId == id);
            DataInitializer.ProductTags.RemoveAll(pt => pt.ProductId == id);
            DataInitializer.Products.Remove(product);
