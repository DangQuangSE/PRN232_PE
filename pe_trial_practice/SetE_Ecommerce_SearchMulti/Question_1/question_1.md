...
Chạy script [`database.sql`](../../../given_pe_trial_practice/SetE_Ecommerce_SearchMulti/1/database.sql) (thư mục given) trên SQL Server trước khi làm bài (tạo database `PE_Practice_EcommerceE` kèm dữ liệu mẫu).
...
The API at url **`/api/designer/update/{id}`**, method **PUT**, updates one designer.

Request body:

```json
{
  "fullName": "Updated Designer",
  "male": true,
  "dob": "1990-10-22",
  "nationality": "USA",
...
- Any exception: status **409 Conflict**, body (plain text): `There is an error while updating.`
