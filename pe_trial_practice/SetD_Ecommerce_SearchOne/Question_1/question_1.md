...
Chạy script [`database.sql`](../../../given_pe_trial_practice/SetD_Ecommerce_SearchOne/1/database.sql) (thư mục given) trên SQL Server trước khi làm bài (tạo database `PE_Practice_EcommerceD` kèm dữ liệu mẫu).
...
The API at url **`/api/designer/delete/{id}`**, method **DELETE**, deletes one designer:

- `{id}` is the designer ID.
- If the designer does not exist: status **404**, body (plain text): `Designer not found.`
- If the designer has one or more products: status **409 Conflict**, body (plain text): `Cannot delete a designer having products.`
